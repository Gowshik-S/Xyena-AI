...
POLICIES: dict[str, dict[str, Any]] = {
    "gst.enterprises.get_classification": {"risk_class": "SENSITIVE_READ"},
    "gst.taxpayers.get": {"risk_class": "SENSITIVE_READ"},
    "gst.registrations.verify": {"risk_class": "SENSITIVE_READ"},
    "gst.invoices.get": {"risk_class": "SENSITIVE_READ"},
    "gst.invoices.verify": {"risk_class": "SENSITIVE_READ"},
    "gst.invoices.search": {"risk_class": "SENSITIVE_READ"},
    "gst.invoices.check_duplicate": {"risk_class": "SENSITIVE_READ"},
    "gst.returns.get_summary": {"risk_class": "SENSITIVE_READ"},
}
...
async def register() -> None:
    ...
    public_url = required("GST_PORTAL_PUBLIC_MCP_URL")
    tenant_ids = [
        value.strip()
        for value in required("GST_PORTAL_TENANT_IDS").split(",")
        if value.strip()
    ]
    hostname = urlparse(public_url).hostname
    if not hostname:
        raise RuntimeError("GST_PORTAL_PUBLIC_MCP_URL must include a hostname.")
    ...
    print(f"Activated {len(POLICIES)} reviewed GST tools for {len(tenant_ids)} tenants.")
...
async def _register_tenant(
    ...
    server = next((item for item in response.json() if item["label"] == "gst"), None)
    if server is not None and (
        server["endpoint"].rstrip("/") != public_url.rstrip("/")
        or server["transport"] != "STREAMABLE_HTTP"
    ):
        raise RuntimeError(f"Tenant {tenant_id} already has a different GST MCP registration.")
    if server is None:
        response = await client.post(
            ...
            json={
                "label": "gst",
                "description": "XYENA synthetic GST and e-Invoice evidence portal",
                "transport": "STREAMABLE_HTTP",
                "endpoint": public_url,
                "auth_type": "BEARER",
                "secret_ref": "env://GST_PORTAL_MCP_TOKEN",
    ...
    if set(latest) != set(POLICIES):
        raise RuntimeError(
            f"GST discovery mismatch for tenant {tenant_id}: "
