...
class GstMcpSecurityError(RuntimeError):
    pass
...
def verify_runtime_scope(ctx: Context, expected_tool: str) -> RuntimeScope:
    ...
    if not isinstance(envelope, dict) or not isinstance(supplied, str):
        raise GstMcpSecurityError("Signed Xyena runtime scope is required.")
    if algorithm != "hmac-sha256":
        raise GstMcpSecurityError("Unsupported runtime signature algorithm.")
    ...
    if not hmac.compare_digest(supplied, expected):
        raise GstMcpSecurityError("Xyena runtime scope signature is invalid.")
    required = {
        "tenant_id",
        "organization_id",
        "user_id",
    ...
    if required - set(envelope):
        raise GstMcpSecurityError("Xyena runtime scope is incomplete.")
    if envelope["canonical_name"] != expected_tool:
        raise GstMcpSecurityError("Runtime tool name does not match this handler.")
    if not str(envelope["purpose"]).strip():
        raise GstMcpSecurityError("A signed purpose is required.")
